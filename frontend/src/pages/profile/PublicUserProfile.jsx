...
export default function PublicProfilePage() {
  ...
  async function fetchProfile() {
    try {
      ...
      if (response?.success) {
        ...
        loadSkills(response.data.skills);
      }
    } catch (error) {
  ...
  async function fetchPosts(pageNum = 0, append = false) {
    try {
      ...
      const response = await getUserPosts(uuid, pageNum, PAGE_SIZE);

      if (response.data?.success) {
        const pageData = response.data.data; // PostPageResponse: { posts, hasNext, ... }
        const newPosts = pageData.posts || [];
  ...
  // Reflect a delete immediately in the list (in case showEditDelete
  // is ever enabled on this page, or reused elsewhere).
